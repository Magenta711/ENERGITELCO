            ...
            <ul>
                @if ($id->minor_box && $id->minor_box->charges > 0)
                    @php
                        $submit = false;
                    @endphp
                    <li>El usuario cuenta con {{ number_format($id->minor_box->charges, 2, ',', '.') }} de caja menor por
                        descargar</li>
                @endif
                @if ($id->minor_box && ($id->minor_box->discharges > 0 || $id->minor_box->pending > 0))
                    @php
                        $submit = false;
                    @endphp
                    <li>El usuario cuenta con
                        ${{ number_format($id->minor_box->discharges + $id->minor_box->pending, 2, ',', '.') }} pendientes
                        por pagarle</li>
                @endif
        ...
        <div class="alert alert-warning alert-dismissible">
            ...
            <ul>
                @if (count($id->unapprovedEvaluation()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedEvaluation()) }} evaluación de desempeño por cerrar
                    </li>
                @endif
                @if (count($id->unapprovedCallAttetion()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedCallAttetion()) }} llamados de atención por cerrar
                    </li>
                @endif
                @if (count($id->unapprovedSeverance()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedSeverance()) }} solicitud de retiro de cesantia o
                        carta laboral por cerrar</li>
                @endif
                ...
                @if (count($id->unapprovedBonuUser()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedBonuUser()) }} reporte de bonificación administrativa
                        sin cerrar</li>
                @endif
                @if (count($id->unapprovedBonuses()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedBonuses()) }} reporte de bonificación administrativa
                        sin aprobar</li>
                @endif
                @if (count($id->unapprovedBoxes()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedBoxes()) }} reporte de bonificación ténicas o caja
                        menor sin aprobar</li>
                @endif
                ...
                @if (count($id->unapprovedPayrollOverTime()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedPayrollOverTime()) }} reporte de pago de nomina sin
                        aprobar</li>
                @endif
                @if (count($id->unapprovedCheckListComputer()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedCheckListComputer()) }} listas de verificación de
                        computadores sin aprobar</li>
                @endif
                @if (count($id->unapprovedDeliveryStaffing()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedDeliveryStaffing()) }} entregas de dotación personal
                        sin aprobar</li>
                @endif
                @if (count($id->unapproveDreviewTool()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapproveDreviewTool()) }} revición y asignación de
                        herramientas sin aprobar</li>
                @endif
                @if (count($id->unapprovedFallProtection()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedFallProtection()) }} equipos de protección contra
                        caidas sin aprobar</li>
                @endif
                ...
                @if (count($id->unapprovedDateilVehicles()) > 0)
                    <li>El usuario cuenta con {{ count($id->unapprovedDateilVehicles()) }} inspección detallada de
                        vehiculos sin aprobar</li>
                @endif
            </ul>
        </div>
        {{-- @endif --}}
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" readonly name="cedula" id="cedula" value="{{ $id->cedula }}"
                                    class="form-control">
                            </div>
                            <div class="col-md-6">
                                <label for="email">Correo</label>
                                <input type="email" readonly name="email" id="email" value="{{ $id->email }}"
                                    class="form-control">
                            </div>
                            <div class="col-md-6">
                                {{-- cedes City --}}
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="date" id="date"
                                    value="{{ $id->register && $id->register->hasContract() ? $id->register->hasContract()->start_date : '' }}"
                                    class="form-control">
                            </div>
                            <div class="col-md-6">
                                <label for="date_end">Fecha de fin</label>
                                <input type="date" name="date_end" id="date_end"
                                    value="{{ $id->register && $id->register->hasContract() && $id->register->hasContract()->date_end ? $id->register->hasContract()->date_end : now()->format('Y-m-d') }}"
                                    class="form-control">
                            </div>
                            <div class="col-md-6">
                                <label for="check_send_email">Enviar correo de finalización</label>
                                <br>
                                <input type="checkbox" name="check_send_email" id="check_send_email" checked="true"
                                    value="1">
                            </div>
                        </div>
                        <hr>
                        ...
                        <ul class="list-group">
                            ...
                            <li class="list-group-item">
                                <label class="form_checked" for="letter_4">
                                    <input type="checkbox" name="letters[]" value="letter_4" id="letter_4"
                                        {{ $monthDff > 2 ? 'checked' : '' }}>

...
@case('1')
        {{" "}}
        <input type="text" value="" name="text[]" placeholder="Respuesta" id="input_{{$question->id}}" class="form-control required {{ $question->required ? 'required' : '' }}">
        @break
    @case('2')
    ...
    @case('4')
        @foreach ($question->options as $option)
            <div class="custom-control custom-checkbox form-check">
                <input type="checkbox" name="checkbox[]" class="custom-control-input checkboxes checkeds_{{$question->id}} {{ $question->required ? 'required' : '' }}" value="{{$option->id}}" id="checkbox_{{$question->id}}_{{$option->id}}">
        ...
        @break
    @case('5')
        <select name="select[]" id="input_{{$question->id}}" class="form-control required {{ $question->required ? 'required' : '' }}">

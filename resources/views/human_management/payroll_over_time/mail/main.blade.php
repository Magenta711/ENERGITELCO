                                ...
                                <tr>
                                    <td class="content-cell" style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; box-sizing: border-box; padding: 35px;">
                                        ...
                                        <p style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji', 'Segoe UI Symbol'; box-sizing: border-box; color: #3d4852; font-size: 16px; line-height: 1.5em; margin-top: 0; text-align: left;">
                                            Estás recibiendo un reporte de novedades y horas extras.</p>
                                            <table class="table-form">
                                                <tbody>
                                                    <tr>
                                                        <th colspan="2">Fechas de corte:</th>
                                                        <th>Valor subsidio de transporte:</th>
                                                    </tr>
                                                    <tr>
                                                        <td>{{ $format->start_date }}</td>
                                                        <td>{{ $format->end_date }}</td>
                                                        <td>
                                                            $ {{ number_format($format->value_assistance, 2, ',', '.') }}
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                            <table class="table-form">
                                                ...
                                                <tbody>
                                                    @foreach ($format->work_adds as $item)
                                                        <tr>
                                            ...
                                            <table class="table-form-1">
                                                ...
                                                <tbody>
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <span id="total_devengado_tx">${{ number_format($format->total_devengado, 0, ',', '.') }}</span>
                                                        </td>
                                                        <td>
                                                            <span id="total_assistance_tx">${{ number_format($format->total_assistance, 0, ',', '.') }}</span>
                                                        </td>
                                                        <td>
                                                            <span id="total_health_tx">${{ number_format($format->total_health, 0, ',', '.') }}</span>
                                                        </td>
                                                        <td>
                                                            <span id="total_pension_tx">${{ number_format($format->total_pension, 0, ',', '.') }}</span>
                                                        </td>
                                                        <td>
                                                            <span id="total_discount_tx">${{ number_format($format->total_discount, 0, ',', '.') }}</span>
                                                        </td>
                                                        <th>
                                                            <span id="total_pay_tx">${{ number_format($format->total_pay, 0, ',', '.') }}</span>
                                                        </th>
                                                    </tr>
                                                </tbody>
                                            </table>
                                            @if ($format->estado != 'Sin aprobar')
                                                <table class="table-form">
                                                    <thead>
                                                        <tr>
                                                            <th>Comentarios</th>
                                            ...
                                            <table class="table-form">
                                                <tbody>
                                                    <tr>
                                                        <td colspan="3">Firmado electrónicamente por quién solicita el
                                                            permiso</td>
                                                        <td colspan="3">Firmado electrónicamente de quién aprueba el permiso
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        ...
                                                        <td colspan="1">Nombre</td>
                                                        <td colspan="2">
                                                            {{ $format->coordinadorAcargo ? $format->coordinadorAcargo->name : '' }}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        ...
                                                        <td colspan="1">Cédula</td>
                                                        <td colspan="2">
                                                            {{ $format->coordinadorAcargo ? $format->coordinadorAcargo->cedula : '' }}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td colspan="3">
                                                            Solicitud elaborada inicialmente y firmada electrónicamente por
                                                            <strong>{{ $format->responsableAcargo->name }}</strong>, en rol
                                                            de {{ $format->responsableAcargo->getRoleNames()[0] }}
                                                            habilitado por Energitelco, con conocimiento de funciones y
                                                            contenido del presente documento. Se cumple Ley 527 de 1999 y
                                                            Decreto 19 de 2012
                                                        </td>
                                                        <td colspan="3">
                                                            @if ($format->coordinadorAcargo)
                                                                Solicitud aprobada y firmada electrónicamente por
                                                                <strong>{{ $format->coordinadorAcargo->name }}</strong> en
                                                                rol de {{ $format->coordinadorAcargo->getRoleNames()[0] }}
                                                                Energitelco, con conocimiento de funciones y contenido del
                                                                presente documento. Se cumple Ley 527 de 1999 y Decreto 19
                                                                de 2012
                                                            @endif
                                                        </td>
                                                    </tr>

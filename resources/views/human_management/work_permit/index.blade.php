    ...
    <section class="content">
        ...
        <div class="box">
            <div class="box-header">
                ...
                <div class="box-tools">
                    <a href="{{ route('work_permit_all') }}" class="btn btn-sm btn-warning">Todos</a>
                        ...
                        <tbody>
                            @foreach ($work_permits as $work_permit)
                                <tr>
                                    ...
                                    <th scope="row">
                                        {{ $work_permit->codigo_formulario ? $work_permit->codigo_formulario . '-' . $work_permit->id : 'N/A' }}
                                    </th>
                                    ...
                                    <td>{{ $work_permit->estado != 'Sin aprobar' && $work_permit->coordinadorAcargo ? $work_permit->coordinadorAcargo->name : '' }}
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <small
                                            class="label {{ $work_permit->estado == 'Sin aprobar' ? 'bg-green' : ($work_permit->estado == 'Aprobado' ? 'bg-blue' : 'bg-red') }}">{{ $work_permit->estado }}</small>
                                    </td>
                                    <td>
                                        @can('Consultar permisos de trabajo')
                                            <a href="{{ route('work_permit_show', $work_permit->id) }}"
                                                class="btn btn-sm btn-success">Ver</a>
                                        @endcan
                                        @if ($work_permit->estado == 'Aprobado')
                                            @can('Descargar PDF de permisos de trabajo')
                                                <a href="{{ route('work_permit_download', $work_permit->id) }}"
                                                    class="btn btn-warning btn-sm">Descargar</a>
                                            @endcan
                                        @endif
                                        @can('Eliminar formato de permisos de trabajo')
                                            <button class="btn btn-sm btn-danger" data-toggle="modal"
                                                data-target="#delete_{{ $work_permit->id }}">Eliminar</button>
                                            <div class="modal fade" id="delete_{{ $work_permit->id }}" tabindex="-1"
                                                ...
                                                <div class="modal-dialog modal-md">
                                                    <div class="modal-content">
                                                        ...
                                                        <form action="{{ route('work_permit_delete', $work_permit->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
